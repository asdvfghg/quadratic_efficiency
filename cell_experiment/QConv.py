...
class ConvTransQuadraticOperation(nn.Module):
    ...
    def reset_parameters(self) -> None:
        init.xavier_uniform_(self.weight_r)
        if self.need_bias:
            self.__reset_bias()

    def forward(self, x):
        if self.need_bias:
            out = F.conv_transpose2d(x, self.weight_r, self.bias_r, self.stride, self.padding, 0, 1) \
              * F.conv_transpose2d(x, self.weight_g, self.bias_g, self.stride, self.padding, 0, 1) \
              + F.conv_transpose2d(torch.pow(x, 2), self.weight_b, self.bias_b, self.stride, self.padding, 0, 1)

        else:
            out = F.conv_transpose2d(x, self.weight_r, None, self.stride, self.padding, 1, 1) \
              * F.conv_transpose2d(x, self.weight_g, None, self.stride, self.padding, 1, 1) \
              + F.conv_transpose2d(torch.pow(x, 2), self.weight_b, None, self.stride, self.padding, 1, 1)
        return out
